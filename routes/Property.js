...
async function saveImageArr({ ImgArr, id, res }) {
	...
	await ImgArr.map(async (a, i) => {
		const image = await SaveImageDB(a, { Property: new mongoose.Types.ObjectId(id) }, res);
		if (image?.file?._id) {
		...
		if (id) {
			...
			if (ImgArr?.length > 0) {
				async function connectImgArrDb() {
					...
					if (uniqueImage.length == newProperty.Image.length) {
						await newProperty.save();
						res.status(200).json({
							status: 200,
		...
		if (id) {
			...
			if (searchProperty?._id) {
				...
				await Property.updateOne(
					...
					{
						...Credentials,
						Tower_FlatNo: `${Credentials?.Tower ? Credentials?.Tower : searchProperty?.Tower}_${
...
router.post("/Property-User", async (req, res) => {
	try {
		...
		const { id, message } = await getAdminId(req);
		if (id) {
			const Check = await CheckAllRequiredFieldsAvailaible(req.body, ["property", "user"], res);
			if (Check) {
				return;
			}
			...
			if (searchProperty?._id && searchUser?._id) {
				...
				User.updateOne(
					...
					.then(async (data) => {
						...
						Property.updateOne(
							...
							.then((data) => {
								res.status(200).json({ status: 200, data: "Property was added to user" });
							})
							.catch((err) => {
								res.status(500).json({ status: 500, message: err });
							});
				...
				res.status(500).json({ status: 500, message: "Propery or User not Found" });
			}
		} else {
			res.status(401).json({ status: 401, message: message });
		}
