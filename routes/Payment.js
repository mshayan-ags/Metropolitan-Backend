...
router.post("/Create-Payment", async (req, res) => {
	try {
		...
		if (id || userId) {
			...
			if (parseInt(searchBill?.TotalAfterDiscount) != parseInt(Credentials?.Amount)) {
				res.status(500).json({
					status: 500,
				...
				searchService?._id &&
				searchBill?._id &&
				parseInt(searchBill?.TotalAfterDiscount) == parseInt(Credentials?.Amount)
			) {
				...
				if (id) {
					newPayment.Admin = new mongoose.Types.ObjectId(id);
				}
