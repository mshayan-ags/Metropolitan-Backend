...
router.post("/Request-Complain", async (req, res) => {
	try {
		...
		if (id || adminID) {
			...
			if (searchComplainCategory?._id && searchProperty?._id && searchUser?._id) {
				const LastEntry = await Complain.findOne({}, {}, { sort: { Number: -1 } });
				...
				const newComplain = new Complain({
					...
					ComplainCategory: new mongoose.Types.ObjectId(searchComplainCategory?._id),
				...
				if (Credentials?.VoiceNote) {
					...
					if (image?.file?._id) {
						newComplain.VoiceNote = new mongoose.Types.ObjectId(image?.file?._id);
					} else {
	...
	try {
		...
		if (id) {
			...
			const updateComplain = {
				status: Credentials?.status ? Credentials?.status : currComplain?.status,
			...
			if (Credentials?.VoiceNote) {
				...
				if (image?.file?._id) {
					updateComplain.VoiceNote = new mongoose.Types.ObjectId(image?.file?._id);
				} else {
					res.status(500).json({ status: 500, message: image?.Error });
				}
			}
